...
apt-get -y install python python-setuptools python-virtualenv ruby ruby-dev rubygems-integration perl build-essential libffi-dev
...
cd /host
fpm -s tar -t deb --name "docker-rpm-builder" --version ${DRB_VERSION} --depends "lxc-docker >= 1.3" ../drb.tar.gz 
chmod 666 *.deb
