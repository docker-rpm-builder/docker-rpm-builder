...
class TestDirCommand(TestCase):
    ...
    def test_source_overlay_option_makes_internal_source_dir_writable_no_external_modification(self):
        ...
        result = self.runner.invoke(dir, [REFERENCE_IMAGE, self.src.path, self.rpm.path, "--enable-source-overlay"],
                           catch_exceptions=False)
        self.assertEquals(0, result.exit_code)
        self.assertFalse(os.path.exists(os.path.join(self.src.path, "aaa")))
